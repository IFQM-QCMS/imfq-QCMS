...
logger = logging.getLogger('qcms.storage_calculator')
    ...
    try:
        ...
        for org in orgs:
            ...
            try:
                wf_bytes = db.session.query(
                    func.sum(func.length(cast(ProjectWorkflow.data, Text)))
            ...
            try:
                audit_bytes = db.session.query(
                    func.sum(
                        ...
                        func.length(cast(AuditLog.details, Text))
                    )
            ...
            try:
                user_bytes = db.session.query(
                    func.sum(
                        ...
                        func.length(cast(User.custom_fields, Text))
                    )
                ).filter(User.org_id == org.id).scalar() or 0
            except Exception:
                try:
            ...
            if not limit_mb:
                stored_lim = getattr(org, 'storage_limit_mb', None)
                limit_mb = stored_lim if (stored_lim and stored_lim > 0) else 10240.0
            ...
            org_data = {
                ...
                "plan": org.subscription_plan or "Trial",
        ...
        return {
            ...
            "summary": {
                ...
                "total_software_used_mb": round(total_orgs_used_mb, 3),
        ...
        logger.error(f"[calculate_org_storage_realtime] Global exception: {e}", exc_info=True)
        try:
            db.session.rollback()
        except Exception:
            pass
        return {
            ...
            "summary": {
                ...
                "total_limit_mb": 10240.0,
                "total_limit_fmt": "10.0 GB",
                "total_orgs": 0,
                "high_usage_count": 0,
                "avg_usage_mb": 0.0
            }
...
def calculate_platform_storage_summary():
    ...
    try:
        ...
        logger.error(f"[calculate_platform_storage_summary] Exception: {e}", exc_info=True)
        try:
            db.session.rollback()
        except Exception:
            pass
