    ...
    try:
        if field_key not in ('email', 'phone', 'username', 'role', 'department', 'plant_location'):
            ...
            if clean_col.lower() not in existing_cols:
                users_table = sa.Table('users', sa.MetaData(), autoload_with=db.engine)
                new_column = sa.Column(clean_col, sa.Text())
                column_ddl = str(CreateColumn(new_column).compile(dialect=db.engine.dialect))
                db.session.execute(sa.text(f'ALTER TABLE users ADD COLUMN {column_ddl}'))
                db.session.commit()
...
def update_user(user_id):
    ...
    # Handle plant first so department lookup in plant can prioritize correctly
    target_pid = None
    if 'plant_id' in data or 'plant' in data or 'plant_location' in data:
        ...
        if pid and str(pid).isdigit():
            target_pid = int(pid)
            user.plant_id = target_pid
        elif pid == '' or pid is None:
            user.plant_id = None

    dept_input = data.get('department') or data.get('dept_name')
    if dept_input is not None:
        if dept_input == '' or dept_input == 'N/A':
            ...
            if not dept:
                query = Department.query.filter_by(name=str(dept_input), org_id=current_user.org_id)
                if user.plant_id:
                    dept = query.filter_by(plant_id=user.plant_id).first()
                if not dept:
                    dept = query.first()

            if not dept and not str(dept_input).isdigit():
                dept = Department(name=str(dept_input), org_id=current_user.org_id, plant_id=user.plant_id)
                db.session.add(dept)
                db.session.flush()

            if dept:
                user.department_id = dept.id
                if dept.plant_id and not user.plant_id:
                    user.plant_id = dept.plant_id

    if not user.plant_id and user.dept and user.dept.plant_id:
        user.plant_id = user.dept.plant_id
