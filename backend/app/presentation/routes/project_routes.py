...
def delete_project(id):
    ...
    try:
        ...
        tables_to_clean = [
            ...
            text("DELETE FROM qc_check_sheet_entries WHERE check_sheet_id IN (SELECT id FROM qc_check_sheets WHERE project_id = :pid)"),
            text("DELETE FROM qc_check_sheet_rows WHERE check_sheet_id IN (SELECT id FROM qc_check_sheets WHERE project_id = :pid)"),
            text("DELETE FROM qc_pareto_items WHERE pareto_chart_id IN (SELECT id FROM qc_pareto_charts WHERE project_id = :pid)"),
            ...
            text("DELETE FROM qc_fishbone_branches WHERE fishbone_id IN (SELECT id FROM qc_fishbone_diagrams WHERE project_id = :pid)"),
            text("DELETE FROM qc_scatter_points WHERE scatter_diagram_id IN (SELECT id FROM qc_scatter_diagrams WHERE project_id = :pid)"),
        ...
        for q in tables_to_clean:
            try:
                with db.session.begin_nested():
                    db.session.execute(q, {"pid": id})
            except Exception as e:
                err_str = str(e).lower()
                ...
                if "does not exist" not in err_str and "no such table" not in err_str and "undefined" not in err_str:
                    import logging
                    logging.warning(f"[delete_project] Failed query: {e}")
